...
// @title LifeRise Vendor API
// @version 1.0
// @description Vendor-facing mobile API for service providers.
// @host vendor-api.liferise.io
// @BasePath /api
func main() {
	...
	v1 := r.Group("/api")
	{
		...
		authRequired.Use(middleware.RequireAuth(jwtService))
		{
			...
			authRequired.POST("/payments/release/:booking_id", paymentHandler.ReleasePayment)
		}
	}

	port := cfg.Server.Port
	if port == 8080 && os.Getenv("PORT") == "" {
		port = 8081 // default vendor-api port when running locally
	}
	...
	go func() {
		logger.Info("starting vendor API server", zap.String("addr", srv.Addr))
	...
	logger.Info("shutting down vendor API server...")
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	if err := srv.Shutdown(ctx); err != nil {
		logger.Fatal("server forced to shutdown", zap.Error(err))
	}
	logger.Info("vendor API server exited gracefully")
}
