...
const PropertyMap = dynamic(() => import('@/components/manager/PropertyMap'), {
  ssr: false,
  loading: () => (
    <div className="h-64 glass rounded-xl flex items-center justify-center text-muted text-sm">
      Loading map…
    </div>
  ),
});
const EngagementChart = dynamic(() => import('@/components/manager/EngagementChart'), {
  ssr: false,
  loading: () => (
      ...
      >
        <div>
          <p className="text-muted text-sm mb-1 flex items-center gap-1.5">
            <Sparkles size={12} className="text-purple-accent" />
            {formatDate()}
          </p>
        ...
        <div className="lg:col-span-3">
          <div className="flex items-end justify-between mb-4">
            ...
            <Link
              ...
              className="text-xs text-purple-accent font-medium flex items-center gap-1 hover:underline"
            >
            ...
            {vendorLeaderboard.map((v, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="col-span-2 text-center text-gold text-sm font-semibold">
                    {v.rating}★
                  </span>
                  <span className="col-span-2 text-right text-teal text-sm font-semibold">
                    {v.earnings}
                  </span>
                  <span className="col-span-1 flex justify-center">
                    <TrendIcon size={13} style={{ color: tc }} />
                  </span>
                </motion.div>
              );
            })}
          ...
          <div className="glass rounded-2xl p-4 mb-4">
            ...
            <div className="flex items-center justify-between">
              <button
                ...
                className={cn(
                  'flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all hover:opacity-90 active:scale-95',
                  sent ? 'bg-teal/20 text-teal' : 'bg-purple-accent text-white'
                )}
              >
                <Send size={14} />
                {sent ? 'Sent ✓' : 'Send'}
              ...
              <Link
                href="/manager/announcements"
                className="text-xs text-muted hover:text-lr-white transition-colors"
              >
                Open full composer →
              </Link>
            </div>
              ...
              <div key={a.id} className="glass rounded-xl p-3 flex items-start gap-3">
                ...
                >
                  {a.urgent ? (
                    ...
                    <Info size={13} className="text-purple-accent" />
                  )}
                </div>
