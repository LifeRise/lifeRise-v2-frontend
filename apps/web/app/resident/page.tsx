...
import {
  ...
  categories,
  events,
  residentBookings as mockResidentBookings,
} from '@/lib/mock-data';
        ...
        {[
          {
            ...
            value: apiBookings.length > 0 ? String(activeBookingsCount) : '3',
            icon: Activity,
            color: 'teal',
          },
          {
            label: 'Pending',
            value: apiBookings.length > 0 ? String(pendingCount) : '1',
            icon: Clock,
            color: 'gold',
          },
          {
            label: 'Completed',
            value: apiBookings.length > 0 ? String(completedCount) : '12',
            icon: CheckCircle,
            color: 'purple',
          },
          {
            label: 'Total Spent',
            value: apiBookings.length > 0 ? `$${totalSpent.toFixed(0)}` : '$840',
            icon: TrendingUp,
            color: 'rose',
          },
        ].map((stat) => {
      ...
      >
        {filteredVendors.map((v) => (
          <motion.div
            key={v.id}
            variants={item}
            whileHover={{ y: -4, boxShadow: '0 0 30px rgba(0,212,170,0.12)' }}
            className="glass rounded-2xl overflow-hidden cursor-pointer group transition-all"
          >
            {/* Card top gradient */}
            <div
              className={`bg-linear-to-br ${v.gradient} h-32 flex items-center justify-center relative`}
            >
              <span className="text-5xl select-none">{v.available ? '✦' : '○'}</span>
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="w-16 h-16 rounded-full bg-white/10 flex items-center justify-center text-2xl font-bold text-white font-heading">
                  {v.initials}
                </span>
              </div>
              {v.badge && (
        ...
        <div>
          ...
          <div className="space-y-3">
            {events.map((e) => (
              ...
              >
                <div
                  className={`bg-linear-to-br ${e.gradient} w-12 h-12 rounded-xl shrink-0 flex items-center justify-center`}
                >
            ...
            {residentBookings.map((b) => {
              ...
              return (
                <div key={b.id} className="glass rounded-xl p-4 flex items-center gap-4">
                  <div
                    className="w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold text-midnight shrink-0"
