...
export default function ServiceCategoriesPage() {
  const [filters, setFilters] = useState({ search: '', active: '', page: 1 });
  const [deleteId, setDeleteId] = useState<number | null>(null);
  const { data, meta, isLoading, error, refresh } = useAdminList<ServiceCategory>(
    'service-categories',
    filters
  );
  const { remove, isPending } = useAdminMutation<ServiceCategory>('service-categories');
  ...
  return (
    <AdminPageShell
      ...
      subtitle={`${meta?.total ?? 0} categories`}
      action={
