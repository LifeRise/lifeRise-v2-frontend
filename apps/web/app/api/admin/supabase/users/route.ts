 ...
 *
 * Server-side proxy that lists staff users (vendor/manager/admin) from the
...
export async function GET(req: NextRequest) {
  ...
  // Base query: non-resident roles
  let query = admin
    .from('profiles')
    .select('*', { count: 'exact' })
    .in('role', ['vendor', 'manager', 'admin']);
  ...
  if (error) {
    console.error('[api/admin/supabase/users] Supabase error:', error.message);
